...
### v1.7.0（当前）
- **角色主题色**：UI 强调色（标题栏底边 / 标题文字 / 主容器边框 / 套路标签）随当前游戏角色自动切换。读取存档识别职业 → WebSocket 推送 → 实时染色，无需手动配置或重启
- **手动选卡覆盖**：点击卡槽即可从 OCR 前 5 个候选（双语 中文/English + 置信度）一键选卡，或从全角色卡库搜索（中英文双向匹配）。修正 OCR 误读 / 模组卡的最后一公里
- **OCR 准确率改进**：
  - 缩小阈值 `_WINRT_MAX_DIM` 1900 → **2300**，1080p~1440p 窗口（如 1936×1119）不再被无谓 downscale，中文细笔画偏旁不丢失
  - 全图 OCR 给出文本但匹配失败的槽位**自动走区域 OCR 重试**（cv2 LANCZOS4 放大 + CLAHE 锐化），常能恢复偏旁细节（如 sammy 的"宇宙冷漠"被读成"阻宙辶漠"的场景）
- **本地诊断与调试（不外发）**：设置对话框新增"🔧 诊断与调试"区块
  - 🔍 查看诊断信息（脱敏视图：用户名 / Steam ID 自动替换为 `<USER>` / `<STEAM_ID>`，可选择查看原始版本）
  - 📁 打开日志目录
  - 📦 打包诊断数据 → 本地 zip（含 app.log / OCR 快照 / 系统信息 + 隐私说明），保存到桌面或自选位置；脱敏开关默认勾选；**不会上传任何地方**
- **OCR 失败截图保存**：识别失败时存到 `logs/ocr_FAIL_*.png`（可关闭，仅本地）；同会话节流避免重复写盘
- **OCR 原文显示开关**：开启后卡槽下方多一行小字 `(原文: 阻宙辶漠)`，方便诊断 OCR 误读
- **文件堆积防控**：app.log 改用 `RotatingFileHandler`（5MB × 2 文件 = ~10MB 上限）；成功快照上限 20 → 10、失败快照上限 → 5
- **UI 提示**：Vision OCR 标题旁追加 `💡 点击卡名手动选择候选`，让新功能可发现
- **字号缩放修复**：Vision OCR 面板内的标签（标题 / 提示 / 卡槽 / 状态）现在跟随设置里的字号滑块实时更新
...
- **4K E_FAIL 修复**：Windows.Media.Ocr 文档规定的 2600px 尺寸硬上限——4K 全屏（3840×2160）会触发 `[WinError -2147467259]` 失败。OCR 前用 INTER_AREA 缩到 2400 长边
